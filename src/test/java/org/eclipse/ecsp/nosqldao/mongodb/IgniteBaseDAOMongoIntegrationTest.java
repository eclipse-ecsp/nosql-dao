...
import org.eclipse.ecsp.nosqldao.ecall.CustomEcu;
import org.eclipse.ecsp.nosqldao.ecall.CustomVehicleProfile;
import org.eclipse.ecsp.nosqldao.ecall.CustomVehicleProfileDao;
...
import org.eclipse.ecsp.nosqldao.ecall.InventoryScomo;
...
import org.eclipse.ecsp.nosqldao.test.TestFirstLvlHierarchy;
import org.eclipse.ecsp.nosqldao.test.TestSecondLvlHierarchy;
...
public class IgniteBaseDAOMongoIntegrationTest {
    ...
    @Qualifier("testFirstLvlHierarchy")
    @Autowired
    TestFirstLvlHierarchy t1;

    @Qualifier("test123")
    @Autowired
    TestSecondLvlHierarchy t2;
    ...
    @Autowired
    private CustomVehicleProfileDao customVehicleProfileDao;
    ...
    @Test
    public void testGetAndUpdateCustomVehicleProfile() {
        CustomVehicleProfile customVehicleProfile = new CustomVehicleProfile();

        customVehicleProfile.setCustomId("123");

        InventoryScomo inventoryScomo = new InventoryScomo();
        inventoryScomo.setScomoId("234");
        inventoryScomo.setVersion("ver_1");

        Map<String, InventoryScomo> innerInventoryMap = new HashMap<>();
        innerInventoryMap.put("key1", inventoryScomo);

        Map<String, Map<String, InventoryScomo>> outerInventoryMom = new HashMap<>();
        outerInventoryMom.put("key2", innerInventoryMap);

        CustomEcu ecu = new CustomEcu();
        ecu.setInventory(outerInventoryMom);

        Map<String, CustomEcu> ecuMap = new HashMap<>();
        ecuMap.put("key3", ecu);

        customVehicleProfile.setEcus(ecuMap);

        customVehicleProfileDao.save(customVehicleProfile);

        //Assert that the map of map type field got saved successfully
        CustomVehicleProfile savedEntity = customVehicleProfileDao.findById("123");
        Assert.assertEquals("234", savedEntity.getEcus().get("key3")
                .getInventory().get("key2").get("key1").getScomoId());

        //Create a new entity, with the same @Id value and update the map of map type field.
        CustomVehicleProfile customVehicleProfile1 = new CustomVehicleProfile();
        customVehicleProfile1.setCustomId("123");
        InventoryScomo inventoryScomo2 = new InventoryScomo();
        inventoryScomo2.setScomoId("456");
        inventoryScomo2.setVersion("ver_2");

        Map<String, InventoryScomo> innerInventoryMap2 = new HashMap<>();
        innerInventoryMap2.put("key1", inventoryScomo2);

        Map<String, Map<String, InventoryScomo>> outerInventoryMom2 = new HashMap<>();
        outerInventoryMom2.put("key2", innerInventoryMap2);
        CustomEcu ecu2 = new CustomEcu();
        ecu2.setInventory(outerInventoryMom2);

        Map<String, CustomEcu> ecuMap2 = new HashMap<>();
        ecuMap2.put("key3", ecu2);

        customVehicleProfile1.setEcus(ecuMap2);
        //Update the entity with @Id = ECallId_Update_1 and assert that ONLY the map of map type field
        //got updated and rest of the other fields remained unchanged.
        customVehicleProfileDao.getAndUpdate(customVehicleProfile1);
        CustomVehicleProfile updatedEntity = customVehicleProfileDao.findById("123");
        Assert.assertEquals("456", updatedEntity.getEcus().get("key3")
                .getInventory().get("key2").get("key1").getScomoId());
        Assert.assertEquals("ver_2", updatedEntity.getEcus().get("key3")
                .getInventory().get("key2").get("key1").getVersion());
